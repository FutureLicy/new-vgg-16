...
class FFTTransform:
    ...
    def __call__(self, img):
        """对图像进行FFT处理，返回9通道张量（3通道RGB + 3通道幅度谱 + 3通道相位谱）"""
        # 先应用基础变换
        ...
        # 沿通道维度拼接：3通道RGB + 3通道幅度谱 + 3通道相位谱
        magnitude_channel = torch.stack(magnitude_channels)  # (3, H, W)
        phase_channel = torch.stack(phase_channels)  # (3, H, W)
        result = torch.cat([img_tensor, magnitude_channel, phase_channel], dim=0)  # (9, H, W)

        # ImageNet标准化（9通道：3通道RGB + 3通道幅度谱 + 3通道相位谱）
...
# ==================== 简化的FFT变换（更快速）===================
class FastFFTTransform:
    """简化的FFT变换，同时提取幅度谱和相位谱"""
    ...
    def __init__(self, resize_size=224):
        ...
        self.base_transform = transforms.Compose([
            transforms.RandomResizedCrop(resize_size),
            transforms.RandomHorizontalFlip(),
            transforms.ToTensor(),  # 直接转换为张量
        ])

    def __call__(self, img):
        """对图像进行FFT处理，返回9通道张量（3通道RGB + 3通道幅度谱 + 3通道相位谱）"""
        # 应用基础变换
        img_tensor = self.base_transform(img)  # (3, H, W)
        ...
        result = torch.cat([img_tensor, magnitude, phase], dim=0)  # (9, H, W)
